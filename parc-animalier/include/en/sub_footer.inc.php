<div id="pre-footer">
	<div class="col_6">
	<h5>Site map</h5>
        <ul class="checks">
        <li><a href="index.php?page=guided_tour&langue=en">Guided tour</a></li>
        <li><a href="index.php?page=access_map&langue=en">How to visit us</a></li>
        <li><a href="index.php?page=park_contact&langue=en">Contact us</a></li>
        <li><a href="index.php?page=legal_notice&langue=en">Legal notice</a></li>
        </ul>	</div>
	
	<div class="col_6">
	<h5>Partners</h5>
        <ul class="checks">
        <li><a href="http://www.les-angles.com/saison/index.php?lang=english" target="_blank">Les Angles' ski resort</a></li>
        <li><a href="http://www.laquillane.fr/" target="_blank">La Quillane' ski resort</a></li>
        <li><a href="http://www.neigescatalanes.com/home.asp?lng=EN_NC" target="_blank">More information about "Les neiges catalanes"</a></li>
        <li><a href="./index.php?page=partners_park&langue=en"><em>All our partners ...</em></a></li>         
        </ul>
	</div>
</div>	
