<div class="col_12">
<!-- Breadcrumbs -->
<ul class="breadcrumbs">
<li><a href="./index.php?page=accueil_parc">Accueil</a></li>
<li>Faune pyrénéenne</li>
</ul>
<p>Venir visiter le parc animalier des Angles, unique dans les Pyrénées, c'est vivre intensément une aventure enrichissante à bien des points de vue. En parcourant les sentiers aménagés du parc animalier, vous découvrirez tous les animaux spécifiques de la <strong>faune pyrénéenne dans leur espace nature montagnard</strong> :<p>

<ul class="tabs left">
<li><a href="#Isard">Isard</a></li>
<li><a href="#Bouquetin">Bouquetin</a></li>
<li><a href="#Cerf">Cerf</a></li>
<li><a href="#Mouflon">Mouflon</a></li>
<li><a href="#Sanglier">Sanglier</a></li>
<li><a href="#Marmotte">Marmotte</a></li>
<li><a href="#Ours">Ours</a></li>
<li><a href="#Loup">Loup</a></li>
<li><a href="#Chevreuil">Chevreuil</a></li>
<li><a href="#Bison">Bison</a></li>
<li><a href="#Daim">Daim</a></li>
<li><a href="#Renne">Renne</a></li>
</ul>

<div id="Isard" class="tab-content">
	<!-- Isard -->
	<?php include($chemin_pages.'/isard_espace_naturel.php'); ?> 	
</div>
<div id="Bouquetin" class="tab-content">
	<!-- Bouquetin -->
	<?php include($chemin_pages.'/bouquetin_espace_naturel.php'); ?> 	
</div>
<div id="Cerf" class="tab-content">
	<!-- Cerf -->
	<?php include($chemin_pages.'/cerf_espace_naturel.php'); ?> 
</div>
<div id="Mouflon" class="tab-content">
	<!-- Mouflon -->
	<?php include($chemin_pages.'/mouflon_espace_naturel.php'); ?> 	
</div>
<div id="Sanglier" class="tab-content">
	<!-- Sanglier -->
	<?php include($chemin_pages.'/sanglier_espace_naturel.php'); ?> 	
</div>
<div id="Marmotte" class="tab-content">
	<!-- Marmotte -->
	<?php include($chemin_pages.'/marmotte_espace_naturel.php'); ?> 	
</div>
<div id="Ours" class="tab-content">
	<?php include($chemin_pages.'/ours_espace_naturel.php'); ?> 
</div>
<div id="Loup" class="tab-content">
	<!-- Loup -->
	<?php include($chemin_pages.'/loup_espace_naturel.php'); ?> 	
</div>	
<div id="Chevreuil" class="tab-content">
	<!-- Chevreuil -->
	<?php include($chemin_pages.'/chevreuil_espace_naturel.php'); ?> 	
</div>
<div id="Bison" class="tab-content">
	<!-- Bison -->
	<?php include($chemin_pages.'/bison_espace_naturel.php'); ?> 	
</div>
<div id="Daim" class="tab-content">
	<!-- Daim -->
	<?php include($chemin_pages.'/daim_espace_naturel.php'); ?> 	
</div>
<div id="Renne" class="tab-content">
	<!-- Renne -->
	<?php include($chemin_pages.'/renne_espace_naturel.php'); ?> 	
</div>

</div>